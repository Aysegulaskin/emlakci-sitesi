...
export default function AdminUserForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!response.ok) {
      ...
      setError(result?.error || "Yönetici oluşturulurken bir hata oluştu.");
      return;
    }
    ...
    setEmail("");
    setPassword("");
    setMessage("Yeni yönetici başarıyla oluşturuldu.");
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-6 text-slate-900">
      ...
      {message && (
        <div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-4 text-emerald-700">{message}</div>
      )}
      ...
      >
        {isLoading ? "Oluşturuluyor..." : "Yeni Yönetici Ekle"}
      </button>
      <p className="text-sm text-slate-500">
        Buradan en fazla 5 adet yönetici hesabı oluşturabilirsiniz.
      </p>
    </form>
  );
}
